...
def summarize_trends(
    ...
    # Load metadata.jsonl
    metadata = {str(item["id"]): item for item in load_dataset("json", data_files=metadata_path, split="train")}

    # Summarizer pipeline (BART-large-CNN, uses GPU if available)
    ...
    cluster_summaries.sort(key=lambda x: -x["total_posts"])
    # Write output
